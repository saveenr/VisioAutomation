...
namespace InfoGraphicsPy
{
    public class PieSliceChart: GridChart
    {
        ...
        public void Draw(Session session)
        {
            ...
            {
                var dp = DataPoints[i];
                var start = VA.Angle.FromRadians(0);
                var end = VA.Angle.FromRadians(System.Math.PI*2.0*normalized_values[i]);
    ...
    public class DoughnutSliceChart : GridChart
    {
        ...
        public void Draw(Session session)
        {
            ...
            for (int i = 0; i < DataPoints.Count; i++)
            {
                ...
                var circle_shape = dom.DrawArc(top_rects[i].Center, radius * 0.7, radius, start, VA.Angle.FromRadians(System.Math.PI*2.0));
                circle_shapes.Add(circle_shape);
