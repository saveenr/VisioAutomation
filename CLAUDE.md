...
**Open-issues backlog pass (2026-05-06).** Substantial cleanup of the long-standing open-issue backlog. Closed: [#105](https://github.com/saveenr/VisioAutomation/issues/105) (the 4.5-year-old "Change Graph Layout/Direction" report; left open for the reporter to confirm but addressed in full via the umbrella below), [#138](https://github.com/saveenr/VisioAutomation/issues/138) (umbrella for #105 follow-ups), [#139](https://github.com/saveenr/VisioAutomation/issues/139) (document the `<directedgraph>` XML format; also picked `<directedgraph>` as the canonical root and added loader validation), [#140](https://github.com/saveenr/VisioAutomation/issues/140) (`connectortype` attribute), [#141](https://github.com/saveenr/VisioAutomation/issues/141) (`direction` + `layout` attributes; also fixed the silent-drop of parsed `usedynamicconnectors` / `scalingfactor` on the way to the renderer), [#128](https://github.com/saveenr/VisioAutomation/issues/128) (`GeometrySection.Render` returned literal 0 instead of new section index), [#129](https://github.com/saveenr/VisioAutomation/issues/129) (`Get-VisioLockCells` `WriteObject` enumerate flag), [#130](https://github.com/saveenr/VisioAutomation/issues/130) (`Get-VisioShape` no `DefaultParameterSetName`), [#142](https://github.com/saveenr/VisioAutomation/issues/142) (narrow positional fix on `Get-VisioMaster -Document`), [#143](https://github.com/saveenr/VisioAutomation/issues/143) (broader positional-parameter audit pass; 11 `Get-*` cmdlets changed; `Get-VisioPage` and `Get-VisioDocument` also picked up `DefaultParameterSetName` as side fixes). Replies on [#102](https://github.com/saveenr/VisioAutomation/issues/102) and [#117](https://github.com/saveenr/VisioAutomation/issues/117) addressed the user reports there; both kept open pending reporter response. PS test count: 4 → 17 (one new `Session.InvokeScript<T>` runspace-test infra helper added in `VTest.PowerShell` is the right tool for any future binder-exercising test).

**Docs work this session:**
- New gitbook page on the .NET side: [Directed graph XML format](https://saveenr.gitbook.io/visioautomation/directed-graph-xml) under a new **Diagram models** section in `SUMMARY.md`. First of the Tier 3 pages.
- New "Templates vs. stencils" section on the PS-side `Open-VisioDocument` page (with `actdir_u.vstx` worked example) + cross-link from `drop-masters.md` (issue #102 follow-up).
- `Custom properties` page on the .NET gitbook fixed: its existing example showed the same buggy `cp.Value = "Hello World"` pattern that bit issue #117. Now calls `EncodeValues()` and has a new "String values are formulas, not literals" subsection.
- New "Adding custom properties to nodes" section on the PS-side `drawing-directed-graphs.md` (issue #117 follow-up).
- Both gitbook `documentation-changes` logs updated; both repos pushed.

**One issue filed for next session:** [#144](https://github.com/saveenr/VisioAutomation/issues/144) — `CustomPropertyCells` values silently round-trip as `0` because they're Visio formulas, not literals. The session's docs patches address the immediate user pain; #144 tracks the API-level fix and lays out four options with a recommended path.

**Next session priorities:**

1. **[#144](https://github.com/saveenr/VisioAutomation/issues/144) — `CustomPropertyCells` auto-encode** *(top priority).* The natural completion of the #117 thread: silent-failure foot-gun where assigning a string `Value` produces a property whose value renders as `0`. Issue body lays out four options with a recommended path (auto-encode in `CustomPropertyHelper.Set` + remove the loader's pre-encode call in the same change). S-M effort, single commit, well-scoped.
2. **Phase 2 prep** — cut the final `VisioAutomation2010` NuGet release per the 2026 refresh plan. Requires answering the deferred version-policy question first (NuGet at `2.6.x` vs PS module at `4.6.1`: converge or stay divergent?). Discussion-first session.
3. **Release CI completion** &mdash; PSGallery publish workflow + nuget.org publish workflow. GitHub Releases workflows ([`release-nuget.yml`](.github/workflows/release-nuget.yml), [`release-psmodule.yml`](.github/workflows/release-psmodule.yml)) already exist for the binary-attachment side; the publish-to-public-feeds piece is still pending. Plan in [`docs/futures/releases.md`](docs/futures/releases.md#automate-releases-via-github-ci-in-progress).
4. **Small follow-ups deferred from the migration** &mdash; the `<frameworkAssembly assemblyName="Microsoft.Office.Interop.Visio" />` line in [`NuGet/VisioAutomation2010.nuspec`](NuGet/VisioAutomation2010.nuspec) is now redundant with the bundled-PIA-via-NuGet behavior (PIA package's `lib/` ships in the .nupkg). Trim as a small standalone commit when convenient.
5. **(Post-2026-10-13) TFM bump** &mdash; only after Windows 10 LTSB 2016 leaves Extended Support. See [enterprise compat memory](../../.claude/projects/C--Users-savee-Documents-GitHub-VisioAutomation/memory/enterprise_compat_ltsb2016.md). Unblocks VS 2026 move.
