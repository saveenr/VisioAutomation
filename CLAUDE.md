...
**This session (2026-05-07b) summary:** Attempted [#163](https://github.com/saveenr/VisioAutomation/issues/163) (pipeline parameter set on `Connect-VisioShape`) and abandoned it after running into a test-infrastructure limitation. The cmdlet itself works (verified manually in PS 5.1), but the regression tests routed through `VTest.PowerShell`'s `InvokeScript<T>` runspace plumbing failed with `CommandTarget: application does not match doc.application`, distinct from the cmdlet logic. Deeper digging surfaced the underlying constraint: `VisioCmdlet : SMA.Cmdlet` (not `PSCmdlet`) was a deliberate choice early-on for testability reasons, and several cmdlet designs (incl. detecting `ParameterSetName`) want PSCmdlet's surface. Rather than work around it for #163 alone, [#163](https://github.com/saveenr/VisioAutomation/issues/163) was moved CY26Q2 &rarr; CY26Q3 and a new investigation issue filed: [#164](https://github.com/saveenr/VisioAutomation/issues/164) "Investigate switching VisioCmdlet base from Cmdlet to PSCmdlet" (CY26Q3, Milestone B). All in-progress code reverted; repo state is clean.
...
Three [CY26Q2 GitHub milestone](https://github.com/saveenr/VisioAutomation/milestone/2) items still open after [#163](https://github.com/saveenr/VisioAutomation/issues/163) was deferred this session (see "Last session" note below). Due 2026-06-30. **Suggested tackle order for next session:**

1. **[#161 &mdash; Version compatibility tables on user-facing gitbooks](https://github.com/saveenr/VisioAutomation/issues/161)**: ~1 day. Two new gitbook pages, one per product (`VisioAutomation_GitBook_Docs` for the NuGet matrix, `VisioPowerShellDocs` on the `visiops_v4_docs` branch for the VisioPS matrix). Per-row data needs sourcing from git tags + csproj `TargetFramework` + `Visio.psd1` `PowerShellVersion` + `Directory.Packages.props` &mdash; no guessing. Best as a focused single-session block with both gitbook checkouts open simultaneously; cross-link the pages and link from `readme.md` + both `CHANGELOG.md`.

**Lower priority for the same semester:**

- **[#152](https://github.com/saveenr/VisioAutomation/issues/152) Visio repo portfolio audit Phase H1**: centralized index + per-repo READMEs across the 9 sibling Visio repos. Bounded but biggest of the open three; Phase H2 (per-repo retire/merge decisions) is its own follow-up. Defer to whenever the smaller items are clear.
...
**Out of scope unless we finish CY26Q2 with bandwidth to spare:** anything in [CY26Q3](https://github.com/saveenr/VisioAutomation/milestone/3), which now also includes [#163](https://github.com/saveenr/VisioAutomation/issues/163) (deferred, parked behind [#164](https://github.com/saveenr/VisioAutomation/issues/164) PSCmdlet investigation) and [#164](https://github.com/saveenr/VisioAutomation/issues/164) itself, in addition to [#162](https://github.com/saveenr/VisioAutomation/issues/162) Phase A scoping.
