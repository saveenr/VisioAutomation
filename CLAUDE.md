...
1. **Phase 2 prep &mdash; cut the final `VisioAutomation2010` NuGet release.** `[Unreleased]` in [`NuGet/CHANGELOG.md`](NuGet/CHANGELOG.md) has accumulated substantial entries (typed setters, `Formula` rename, `ArgumentException` wrapping, directed-graph XML attribute work). Discussion-first: the deferred version-policy question (NuGet at `2.6.x` vs PS module now at `4.7.2`: converge or stay divergent?) needs an answer before pulling the trigger. With `publish-nuget.yml` now landed, the actual release is two-click (run `release-nuget.yml`, then `publish-nuget.yml` against the resulting tag), modulo the `NUGET_API_KEY` repo-secret setup gate.
