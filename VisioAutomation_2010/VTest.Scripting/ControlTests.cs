...
namespace VTest.Scripting
{
    ...
    {
        [MUT.TestMethod]
        public void GetControls_OnFreshShapes_ReturnsZeroPerShape()
        {
            ...
            var controls = client.Control.GetControls(VisioScripting.TargetShapes.Auto, VisioAutomation.Core.CellValueType.Formula);
            MUT.Assert.AreEqual(3, controls.Count);
            MUT.Assert.AreEqual(0, controls[s1].Count);
            MUT.Assert.AreEqual(0, controls[s2].Count);
            MUT.Assert.AreEqual(0, controls[s3].Count);

            client.Document.CloseDocument(VisioScripting.TargetDocuments.Auto);
        }

        [MUT.TestMethod]
        public void AddControlToShapes_OnSelectedShapes_AppendsOneControlPerShape()
        {
            var client = this.GetScriptingClient();
            client.Document.NewDocument();
            client.Page.NewPage(VisioScripting.TargetDocument.Auto, new VisioAutomation.Core.Size(4, 4), false);
            ...
            var controls = client.Control.GetControls(VisioScripting.TargetShapes.Auto, VisioAutomation.Core.CellValueType.Formula);
            MUT.Assert.AreEqual(3, controls.Count);
            MUT.Assert.AreEqual(1, controls[s1].Count);
            MUT.Assert.AreEqual(1, controls[s2].Count);
            MUT.Assert.AreEqual(1, controls[s3].Count);

            client.Document.CloseDocument(VisioScripting.TargetDocuments.Auto);
        }

        [MUT.TestMethod]
        public void DeleteControlWithIndex_OnShapesWithOneControl_RemovesAllControls()
        {
            var client = this.GetScriptingClient();
            client.Document.NewDocument();
            client.Page.NewPage(VisioScripting.TargetDocument.Auto, new VisioAutomation.Core.Size(4, 4), false);
