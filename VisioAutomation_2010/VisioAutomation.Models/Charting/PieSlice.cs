...
namespace VisioAutomation.Models.Charting
{
    public class PieSlice
    {
        ...
        public VisioAutomation.Geometry.Point Center { get; }
        ...
        public PieSlice(VisioAutomation.Geometry.Point center, double start, double end)
        {
            this.Center = center;
        ...
        public PieSlice(VisioAutomation.Geometry.Point center, double radius, double start, double end) :
        ...
        public PieSlice(VisioAutomation.Geometry.Point center, double start, double end, double inner_radius, double radius) :
            this(center,start,end)
        {
            if (inner_radius < 0.0)
            {
        ...
        internal List<VisioAutomation.Geometry.Point> GetShapeBezierForPie(out int degree)
        {
            ...
            var pie_bez = new List<VisioAutomation.Geometry.Point>(3 + arc_bez.Count + 3);
        ...
        public IVisio.Shape RenderPie( IVisio.Page page)
        {
            ...
            else if (this.Angle >= 2*System.Math.PI)
            {
                ...
                var rect = new VisioAutomation.Geometry.Rectangle(a, b);
            ...
            else
            {
                ...
                // Render the bezier
                var doubles_array = VisioAutomation.Geometry.Point.ToDoubles(pie_bez).ToArray();
        ...
        public IVisio.Shape RenderDoughnut(IVisio.Page page)
        {
            ...
            {
                var outer_radius_point = new VisioAutomation.Geometry.Point(this.Radius, this.Radius);
                var c = this.Center - outer_radius_point;
                var d = this.Center + outer_radius_point;
                var outer_rect = new VisioAutomation.Geometry.Rectangle(c, d);

                var inner_radius_point = new VisioAutomation.Geometry.Point(this.InnerRadius, this.InnerRadius);
                var a = this.Center - inner_radius_point - c;
                var b = this.Center + inner_radius_point - c;
                var inner_rect = new VisioAutomation.Geometry.Rectangle(a, b);
            ...
            else
            {
                ...
                // Render the bezier
                var doubles_array = VisioAutomation.Geometry.Point.ToDoubles(thickarc).ToArray();
                var pie_slice = page.DrawBezier(doubles_array, (short)degree, 0);
                return pie_slice;
            }
        }

        List<VisioAutomation.Geometry.Point> GetShapeBezierForDoughnut(out int degree)
        {
            this.check_normal_angle();
            ...
            // Create one big bezier that accounts for the entire pie shape. This includes the arc
            // calculated above and the sides of the pie slice
            var bez = new List<VisioAutomation.Geometry.Point>(3 + bez_inner.Count + 3);
        ...
        public static List<PieSlice> GetSlicesFromValues(VisioAutomation.Geometry.Point center, double radius, IList<double> values)
        {
        ...
        public static List<PieSlice> GetSlicesFromValues(VisioAutomation.Geometry.Point center, double inner_radius, double outer_radius, IList<double> values)
        {
        ...
        protected VisioAutomation.Geometry.Point GetPointAtRadius(VisioAutomation.Geometry.Point origin, double angle, double radius)
        {
            double x = radius * System.Math.Cos(angle);
            double y = radius * System.Math.Sin(angle);
            var new_point = new VisioAutomation.Geometry.Point(x, y);
            new_point = origin + new_point;
            return new_point;
        }

        protected List<VisioAutomation.Geometry.Point> GetArcBez(double radius, out int degree)
        {
            // split apart the arc into distinct bezier segments (will end up with at least 1 segment)
            // the segments will "fit" end to end
            var sub_arcs = Geometry.BezierSegment.FromArc(
                this.SectorStartAngle,
                this.SectorEndAngle);

            // merge bezier segments together into a list of points
            var merged_points = Geometry.BezierSegment.Merge(sub_arcs, out degree);

            var arc_bez = new List<VisioAutomation.Geometry.Point>(merged_points.Count);
