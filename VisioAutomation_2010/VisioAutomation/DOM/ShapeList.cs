    ...
    public class ShapeList : Node, IEnumerable<BaseShape>
    {
        ...
        private void _draw_non_masters(RenderContext ctx, List<BaseShape> non_masters)
        {
            foreach (var shape in non_masters)
            {
                ...
                {
                    var ps = (Arc)shape;
                    var vad_arcslice = new VA.Layout.Models.Charting.DoughnutSlice(ps.Center, ps.StartAngle,
                ...
                else if (shape is PieSlice)
                {
                    ...
                    var vad_ps = new VA.Layout.Models.Charting.PieSlice(ps.Center, ps.Start, ps.End, ps.Radius);
