...
namespace VisioAutomation.Application
{
    public static class UserInterfaceHelper
    {
            ...
            System.Windows.Forms.Form the_form)
        {
            ...
            const int window_prop_index = NativeMethods.GWL_STYLE;
            const int window_prop_value = NativeMethods.WS_CHILD | NativeMethods.WS_VISIBLE;
            NativeMethods.SetWindowLong(hwnd_as_intptr, window_prop_index, window_prop_value);

            // Set the anchor bar window as the parent of the form.
            NativeMethods.SetParent(hwnd, anchor_window.WindowHandle32);
