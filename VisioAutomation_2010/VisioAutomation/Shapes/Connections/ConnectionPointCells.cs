...
namespace VisioAutomation.Shapes.Connections
{
    public class ConnectionPointCells : VA.ShapeSheet.CellGroups.CellGroupMultiRow
    {
        ...
        {
            var query = get_query();
            return ____GetCells<ConnectionPointCells,double>(page, shapeids, query, query.GetCells);
        }

        public static IList<ConnectionPointCells> GetCells(IVisio.Shape shape)
        {
            var query = get_query();
            return ____GetCells<ConnectionPointCells,double>(shape, query, query.GetCells);
        }
