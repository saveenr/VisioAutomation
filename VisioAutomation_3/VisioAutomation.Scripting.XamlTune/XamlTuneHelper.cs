...
namespace VisioAutomation.Scripting.XamlTune
{
    public static class XamlTuneHelper
    {
        public static void ExportSelectionAsXAML(Session scripting_session, string filename)
        {
            ...
            load_svg_timer.Stop();
            scripting_session.Write(OutputStream.Verbose, "Finished SVG Loading ({0} seconds)", load_svg_timer.Elapsed.TotalSeconds);

            // Delete temp SVG
            scripting_session.Export.DeleteTemporarySVG(svg_filename);

            scripting_session.Write(OutputStream.Verbose, "Creating XHTML with embedded SVG");
            var s = svg_filename;

            if (System.IO.File.Exists(filename))
            {
                scripting_session.Write(OutputStream.Verbose, "Deleting \"{0}\"", filename);
                System.IO.File.Delete(filename);
            }

            scripting_session.Write(OutputStream.Verbose, "Converting to XAML ...");
            ...
            catch (System.Exception e)
            {
                ...
                scripting_session.Write(OutputStream.Error, msg);
                return;
            }
            convert_timer.Stop();

            scripting_session.Write(OutputStream.Verbose, "Writing XAML File");
            System.IO.File.WriteAllText(filename, xaml);
            scripting_session.Write(OutputStream.Verbose, "Finished writing XAML File");

            scripting_session.Write(OutputStream.Verbose,
