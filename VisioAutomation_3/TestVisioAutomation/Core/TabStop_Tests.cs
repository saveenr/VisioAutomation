...
namespace TestVisioAutomation
{
    ...
    public class TabStop_Tests : VisioAutomationTest
    {
        ...
        public void SetTabStops()
        {
            ...
            Assert.AreEqual(0, VA.Text.TabStop.GetTabStopCount(s1));

            // clearing tab stops shoudl work even if there are no tab stops
            VA.Text.TabStop.SetTabStops(s1, no_tab_stops);
            Assert.AreEqual(0, VA.Text.TabStop.GetTabStopCount(s1));

            // set the 3 tab stops
            VA.Text.TabStop.SetTabStops(s1, tabstops);

            // should have exactly the same number we set
            Assert.AreEqual(tabstops.Length, VA.Text.TabStop.GetTabStopCount(s1));
