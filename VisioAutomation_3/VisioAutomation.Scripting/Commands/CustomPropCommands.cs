...
{
    public class CustomPropCommands : SessionCommands
    {
        ...
        public IDictionary<IVisio.Shape, Dictionary<string,VA.CustomProperties.CustomPropertyCells>> GetCustomProperties()
        {
            ...
            var list_custom_props = CustomPropertyHelper.GetCustomProperties(page, shapes);
        ...
        public IList<bool> HasCustomProperty(string name)
        {
            ...
            var results = (from s in this.Session.Selection.EnumSelectedShapes()
                           select CustomPropertyHelper.HasCustomProperty(s, name))
                .ToList();
        ...
        public void DeleteCustomProperty(string name)
        {
            ...
            using (var undoscope = application.CreateUndoScope())
            {
                ...
                {
                    CustomPropertyHelper.DeleteCustomProperty(shape, name);
                }
            }
        }
        ...
        public void SetCustomProperty(string name, VA.CustomProperties.CustomPropertyCells customprop)
        {
            ...
            using (var undoscope = application.CreateUndoScope())
            {
                foreach (var shape in shapes)
                {
                    CustomPropertyHelper.SetCustomProperty(shape, name, customprop);
                }
            }
        }
