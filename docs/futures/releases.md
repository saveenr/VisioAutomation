...
### Reconcile version numbers across artifacts *(decision: stay divergent; re-evaluated 2026-05-09 at the PS-5.1 manifest bump)*
- **What:** The NuGet [`VisioAutomation2010.nuspec`](../../NuGet/VisioAutomation2010.nuspec) is at `3.0.0`; the PowerShell [`Visio.psd1`](../../VisioAutomation_2010/VisioPowerShell/Visio.psd1) is at `4.7.2`; csproj `AssemblyVersion`s are independent.
...
- **Re-evaluation (2026-05-09):** The PS-5.1 manifest bump landed in [#178](https://github.com/saveenr/VisioAutomation/issues/178); per the decision-revisit trigger above, version policy was reconsidered alongside the manifest cleanup. **Decision: stay divergent.** Same reasoning: the divergence is established, downstream consumers and bug reports reason about the two version lines independently, and the saveenr &rarr; SevenPens identity transition + the VisioScripting public-API decision both shipped under the existing version lines. Forcing a converge mid-rebrand was rejected.
- **Initial decision (2026-05-07):** Stay divergent. NuGet continues on the `3.x` line (was on `2.x` until the [3.0.0] release on 2026-05-07), PS module on the `4.x` line. Trigger for revisit: the [`Visio.psd1` deprecation warnings](#address-visiopsd1-deprecation-warnings-on-psgallery-publish) work, which is itself a "we are revisiting Visio.psd1's declared metadata" event.
- **Next release versions (planned 2026-05-09):**
  - **NuGet `VisioAutomation2010` 3.0.0 &rarr; 3.1.0.** Minor bump. The `[Unreleased]` block in [`NuGet/CHANGELOG.md`](../../NuGet/CHANGELOG.md) is dominated by additions (4 new facade methods on `Client.Model` / `Client.Page` / `Client.Selection` per the Phase B [#182](https://github.com/saveenr/VisioAutomation/issues/182) work) plus several internalizations under "Removed" that are technically breaking but with zero known external consumers per the [#156](https://github.com/saveenr/VisioAutomation/issues/156) usage audit. SemVer-strictly the internalizations would push to `4.0.0`; in practice no consumer sees the break, so minor.
  - **PS module `Visio` 4.7.2 &rarr; 4.7.3.** Patch bump. The `[Unreleased]` block in [`VisioPowerShell/CHANGELOG.md`](../../VisioAutomation_2010/VisioPowerShell/CHANGELOG.md) is the SevenPens brand swap, the [#177](https://github.com/saveenr/VisioAutomation/issues/177) Release-build switch, and the [#178](https://github.com/saveenr/VisioAutomation/issues/178) manifest cleanup. None of these is a behavior change visible to a cmdlet user. The bundled DLLs do change (this PS release will pick up the Phase B + C library work), but the cmdlet surface itself is identical.
- **History of options considered:**
  - **A &mdash; Converge:** both artifacts ship at the same number going forward. Rejected at both decision points.
  - **B &mdash; Document the divergence policy:** keep them independent, write down the rule. **Chosen** &mdash; this section *is* the documentation.
  - **C &mdash; Single technical source of truth:** `Directory.Build.props` + token substitution into nuspec/psd1. Re-evaluate when there's a forcing function (e.g. tooling pain that the divergence specifically causes). Not yet.
- **Cross-refs:** *Automate releases via GitHub CI* below already handles two version sources (each `release-*.yml` reads its own version from its own metadata file), so no workflow changes are required.
