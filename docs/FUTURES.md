...
- **Baseline (2026-05-04, before any further cleanup work)** &mdash; all 163 tests across the four projects pass cleanly (VTest 94, VTest.Models 37, VTest.Scripting 28, VTest.PowerShell 4). Got there in two waves:
  ...
  - **Last failing test (`Dom_DrawOrgChart`)** &mdash; fixed by version-guarding the template filename. Visio 2013 (v15) replaced binary `.vst` templates with XML-based `.vstx`; the test was hardcoding `orgchart.vst`, which doesn't exist on modern Visio installs. The test already version-guarded the master name (`"Position Belt"` for v15+ vs `"Position"` for v14); the fix mirrors that pattern for the template (`orgch_u.vstx` for v15+ vs `orgchart.vst` for v14). **Production-code parallel still open:** [`OrgChartStyling.cs`](../VisioAutomation_2010/VisioAutomation.Models/Documents/OrgCharts/OrgChartStyling.cs) has `Visio2013Template = "orgch_u.vst"` — same bug pattern, would also fail on modern Visio. No test caught it because `OrgChartDocument`'s default-styling path isn't exercised end-to-end. Worth fixing (and adding test coverage for) before the Phase 2 NuGet release.
