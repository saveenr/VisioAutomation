...
- **Step 2 (remaining):** Bump the shipping fleet again to clear the **VS 2026** floor (Framework 4.6.2 minimum). Recommended landing point: **4.7.2** — same TFM the test projects already use, so the whole solution converges on one number. **When step 2 happens, also delete:** the `Microsoft.NETFramework.ReferenceAssemblies.net45` entries in each `packages.config` and the per-csproj `<Import>` + `EnsureNuGetPackageBuildImports` target blocks that reference the package (one block at the bottom of each of the six net45 csprojs) — all temporary scaffolding to compensate for the missing v4.5 targeting pack on modern Windows install media (see [BUILDING.md](BUILDING.md#note-on-the-v45-reference-assemblies-nuget)). The v4.7.2 reference assemblies ship in-box, so none of this is needed once we move. See *Move development to Visual Studio 2026* below.
